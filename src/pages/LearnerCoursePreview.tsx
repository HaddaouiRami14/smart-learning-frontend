...
const LearnerCoursePreview = () => {
  ...
  // ✅ CHANGED: removed enrollUser() — redirect to /enroll if not enrolled
  const loadEnrollment = useCallback(async (sortedChapters: Chapter[]) => {
    ...
    try {
      ...
      // ✅ Not enrolled → redirect to payment/enroll page
      navigate(`/courses/${courseId}/enroll`);
    }
  }, [userId, courseId, applyProgressDetail, navigate]);
  ...
  return (
    <div className="min-h-screen bg-background">
          ...
          <div className="flex items-center gap-3">
            {progressPercent === 100 ? (
              <>
                ...
                <button
                  onClick={() => { setPreviewMode(true); setSelectedChapter(chapters[0] || null); }}
      ...
      <div className="relative w-full bg-gradient-to-r from-gray-900 via-slate-800 to-gray-900 overflow-hidden">
        {course.imageUrl && (
          <div className="absolute inset-0 bg-cover bg-center opacity-20" style={{ backgroundImage: `url(${course.imageUrl})` }} />
        )}
        <div className="relative max-w-7xl mx-auto px-8 py-10">
          <div className="flex items-start justify-between gap-8">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-4">
                <span className="px-3 py-1 bg-white/10 text-white text-xs font-semibold rounded-full border border-white/20">{course.category}</span>
                {levelInfo && <span className={`px-3 py-1 text-xs font-semibold rounded-full border ${levelInfo.color}`}>{levelInfo.label}</span>}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-5 text-sm text-gray-400">
                ...
                <span className="flex items-center gap-1.5"><Clock className="w-4 h-4 text-purple-400" />Updated {new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 flex-shrink-0">
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl px-8 py-5 text-center min-w-[160px]">
                ...
                {course.price === 0 ? (
                  ...
                  <p className="text-2xl font-extrabold text-white">${course.price.toFixed(2)}<span className="text-sm font-normal text-gray-400 ml-1">USD</span></p>
                )}
              </div>
              <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl px-8 py-5 text-center min-w-[160px]">
                {progressPercent === 100 ? (
                  <>
                    ...
                    <div className="flex flex-col items-center gap-2">
                      <div className="w-12 h-12 rounded-full bg-green-500/20 flex items-center justify-center"><Award className="w-6 h-6 text-green-400" /></div>
                  ...
                  <>
                    ...
                    <div className="w-full h-2 bg-white/20 rounded-full mt-2 overflow-hidden">
                      <div className="h-full bg-indigo-400 rounded-full transition-all duration-500" style={{ width: `${progressPercent}%` }} />
                    </div>
                  ...
                  >
                    <div className="flex gap-4 items-center">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`font-semibold mb-1 ${isSelected ? 'text-indigo-600' : isUnlocked ? 'text-foreground' : 'text-muted-foreground'}`}>
                          {chapter.title}
                        </p>
                        {!isUnlocked && <p className="text-xs text-muted-foreground">Complete previous chapter to unlock</p>}
                        {isUnlocked && (
                          <div className="flex items-center gap-2 text-xs">
                            {chapterQuizPassed[chapter.id] && <span className="text-green-600 flex items-center gap-0.5"><CheckCircle className="w-3 h-3" /> Quiz</span>}
                            {chapterExercisePassed[chapter.id] && <span className="text-green-600 flex items-center gap-0.5"><CheckCircle className="w-3 h-3" /> Exercise</span>}
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <>
              {previewMode && (
                <div className="flex items-center justify-between px-6 py-4 bg-green-500/10 border-2 border-green-300 dark:border-green-700 rounded-2xl">
                  ...
                  <button onClick={() => setPreviewMode(false)} className="flex items-center gap-2 px-4 py-2 bg-muted hover:bg-muted/70 text-foreground rounded-lg text-sm font-medium transition-colors border border-border">
                    <XCircle className="w-4 h-4" /> Exit Preview
                  </button>
                </div>
              )}
              ...
              <div className="bg-card rounded-2xl shadow-xl p-10 border border-border prose dark:prose-invert max-w-none"
              ...
              {resources.length > 0 && (
                <div className="bg-card rounded-2xl shadow-xl p-8 border-2 border-blue-200 dark:border-blue-900">
                  <h3 className="text-xl font-bold text-foreground mb-4 flex items-center gap-2">
                    ...
                    Chapter Resources
                    <span className="ml-2 px-2 py-0.5 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded-full text-sm font-medium">{resources.length}</span>
                  </h3>
                  <div className="space-y-3">
                    {resources.map((resource) => (
                      <div key={resource.id} className="flex items-center justify-between p-4 bg-muted rounded-xl border border-border hover:bg-muted/70 transition-colors">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs text-muted-foreground mt-0.5">{resource.resourceType} • {formatFileSize(resource.fileSize)}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 flex-shrink-0 ml-4">
                          {['PDF', 'IMAGE', 'VIDEO', 'AUDIO'].includes(resource.resourceType) && (
                            <button onClick={() => handlePreviewResource(resource)} className="flex items-center gap-2 px-4 py-2 text-sm text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900 transition-colors font-medium">
                              <Eye className="w-4 h-4" /> Preview
                            </button>
                          )}
                          <button onClick={() => handleDownloadResource(resource.id, resource.fileName)} className="flex items-center gap-2 px-4 py-2 text-sm text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-950 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900 transition-colors font-medium">
                  ...
                  {previewResource && (
                    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={closePreview}>
                      <div className="bg-card rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-muted rounded-t-2xl flex-shrink-0">
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-xs text-muted-foreground">{previewResource.resourceType} • {formatFileSize(previewResource.fileSize)}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0 ml-4">
                            <button onClick={() => handleDownloadResource(previewResource.id, previewResource.fileName)} className="flex items-center gap-2 px-3 py-2 text-sm text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-950 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900 transition-colors font-medium">
                              <Download className="w-4 h-4" /> Download
                            </button>
                            <button onClick={closePreview} className="p-2 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-colors">
                        ...
                        <div className="flex-1 overflow-auto flex items-center justify-center bg-muted min-h-0">
                          {loadingPreview ? (
                            ...
                            <>
                              {previewResource.resourceType === 'PDF' && <iframe src={previewUrl} className="w-full h-full min-h-[70vh]" title={previewResource.fileName} />}
                              {previewResource.resourceType === 'IMAGE' && <img src={previewUrl} alt={previewResource.fileName} className="max-w-full max-h-[75vh] object-contain rounded-lg shadow-md m-4" />}
                              {previewResource.resourceType === 'VIDEO' && <video src={previewUrl} controls autoPlay className="max-w-full max-h-[75vh] rounded-lg shadow-md m-4">Your browser does not support video playback.</video>}
                              {previewResource.resourceType === 'AUDIO' && (
                                <div className="flex flex-col items-center gap-6 py-16 px-8">
                                  ...
                                  <audio src={previewUrl} controls autoPlay className="w-full max-w-md">Your browser does not support audio playback.</audio>
                                </div>
                              )}
                            </>
                          ) : (
              ...
              {quiz && (
                <div id="quiz-section" className="bg-card rounded-2xl shadow-xl p-8 border-2 border-purple-200 dark:border-purple-900">
                  <div className="flex items-center gap-3 mb-6">
                    <div className="p-3 bg-purple-100 dark:bg-purple-950 rounded-xl"><HelpCircle className="w-8 h-8 text-purple-600" /></div>
                    <div>
                  ...
                  {currentChapterQuizPassed && !quizResult && !showQuiz ? (
                    <div className="flex flex-col items-center py-8 gap-3">
                      <div className="flex items-center justify-center w-16 h-16 rounded-full bg-green-100 dark:bg-green-950"><CheckCircle className="w-9 h-9 text-green-600" /></div>
                      ...
                      <button onClick={() => setShowQuiz(true)} className="mt-2 px-6 py-2 bg-purple-100 dark:bg-purple-950 text-purple-700 dark:text-purple-300 rounded-lg font-medium hover:bg-purple-200 dark:hover:bg-purple-900 transition-colors">Retake Anyway</button>
                    </div>
                  ) : !showQuiz && !quizResult ? (
                    <div className="text-center py-8">
                      <button onClick={() => setShowQuiz(true)} className="px-8 py-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl font-semibold hover:from-purple-700 hover:to-pink-700 transition-all">Start Quiz</button>
                    </div>
                  ) : showQuiz && !quizResult ? (
                    <div className="space-y-6">
                        ...
                        <div key={q.id} className="border-2 border-purple-100 dark:border-purple-900 rounded-xl p-6 bg-card">
                          <div className="flex items-start gap-3 mb-4">
                            ...
                            <div className="flex-1">
                              ...
                              <span className={`inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-medium ${getQuestionTypeColor(q.questionType)}`}>{getQuestionTypeLabel(q.questionType)}</span>
                            </div>
                          </div>
                          ...
                          {q.questionType === 'SHORT_ANSWER' && (
                            <div className="ml-12">
                              <div className="flex items-center gap-2 mb-2 text-amber-600"><Type className="w-4 h-4" /><span className="text-sm font-medium">Enter your short answer below:</span></div>
                              <input type="text" value={quizTextAnswers[q.id] || ''} onChange={(e) => handleTextAnswer(q.id, e.target.value)} placeholder="Type your answer here..." className="w-full px-4 py-3 border-2 border-amber-200 dark:border-amber-800 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-background text-foreground" />
                            </div>
                          )}
                          {q.questionType === 'EDITOR_ANSWER' && (
                            <div className="ml-12">
                              <div className="flex items-center gap-2 mb-2 text-teal-600"><FileTextIcon className="w-4 h-4" /><span className="text-sm font-medium">Write your detailed answer below:</span></div>
                              <div className="border-2 border-teal-200 dark:border-teal-800 rounded-lg overflow-hidden [&_.ck-editor__editable]:!bg-background [&_.ck-editor__editable]:!text-foreground [&_.ck-toolbar]:!bg-muted [&_.ck-toolbar]:!border-border">
                                <CKEditor
                                  ...
                                  config={{ licenseKey: 'GPL', toolbar: ['heading', '|', 'bold', 'italic', 'underline', '|', 'link', '|', 'bulletedList', 'numberedList', '|', 'blockQuote', '|', 'undo', 'redo'] }}
                                />
                              </div>
                            </div>
                          )}
                      ...
                      <button onClick={submitQuiz} disabled={submittingQuiz} className="w-full px-8 py-4 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl font-semibold hover:from-purple-700 hover:to-pink-700 transition-all disabled:opacity-50">
                  ...
                  {quizResult && (
                    <div className="mt-6 p-6 rounded-xl border-2 border-purple-200 dark:border-purple-900 bg-card">
                          ...
                          return (
                            <div key={r.questionId} className={`p-4 rounded-lg border-2 ${r.isCorrect ? 'border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950' : 'border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950'}`}>
                              <div className="flex items-start gap-3">
                                {r.isCorrect ? <CheckCircle className="w-6 h-6 text-green-600 flex-shrink-0" /> : <XCircle className="w-6 h-6 text-red-600 flex-shrink-0" />}
                                <div className="flex-1">
                                  <p className="font-medium text-foreground">{idx + 1}. {r.questionText}</p>
                                  {question?.questionType === 'MULTIPLE_CHOICE' && (<>
                                    <p className="text-sm text-muted-foreground mt-1">Your answers: {(r.selectedOptionIds || []).length > 0 ? (r.selectedOptionIds || []).map(id => question.options.find(o => o.id === id)?.optionText).filter(Boolean).join(', ') : 'Not answered'}</p>
                                    {!r.isCorrect && <p className="text-sm text-green-600 dark:text-green-400 font-medium">Correct answers: {(r.correctOptionIds || []).map(id => question.options.find(o => o.id === id)?.optionText).filter(Boolean).join(', ')}</p>}
                                  </>)}
                                  {question?.questionType === 'TRUE_FALSE' && (<>
                                    <p className="text-sm text-muted-foreground mt-1">Your answer: {question.options.find(o => o.id === r.selectedOptionId)?.optionText || 'Not answered'}</p>
                                    {!r.isCorrect && <p className="text-sm text-green-600 dark:text-green-400 font-medium">Correct answer: {question.options.find(o => o.id === r.correctOptionId)?.optionText}</p>}
                                  </>)}
                                  {(question?.questionType === 'SHORT_ANSWER' || question?.questionType === 'EDITOR_ANSWER') && (<>
                                    <p className="text-sm text-muted-foreground mt-1">Your answer: <span dangerouslySetInnerHTML={{ __html: r.textAnswer || 'Not answered' }} /></p>
                                    {!r.isCorrect && <p className="text-sm text-green-600 dark:text-green-400 font-medium">Expected answer: <span dangerouslySetInnerHTML={{ __html: r.correctAnswer || '' }} /></p>}
                                  </>)}
                                </div>
                              </div>
                            </div>
                      ...
                      <button onClick={retakeQuiz} className="w-full mt-6 px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl font-semibold hover:from-purple-700 hover:to-pink-700 transition-all flex items-center justify-center gap-2">
              ...
              {exercise && (
                <div className="bg-card rounded-2xl shadow-xl p-8 border-2 border-indigo-200 dark:border-indigo-900">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="p-3 bg-indigo-100 dark:bg-indigo-950 rounded-xl"><Code className="w-8 h-8 text-indigo-600" /></div>
                    <div className="flex-1">
                      ...
                      <button onClick={() => setShowHints(!showHints)} className="mt-1 text-sm text-blue-600 font-medium flex items-center gap-1">
                        <Lightbulb className="w-4 h-4" />{showHints ? 'Hide Hints' : 'Show Hints'}
                      </button>
                      {showHints && <p className="mt-1 text-foreground bg-blue-50 dark:bg-blue-950 p-2 rounded">{exercise.hints}</p>}
                    </div>
                  </div>
                  <Editor height="300px" language={getLanguageForMonaco(exercise.language)} value={exerciseCode} onChange={setExerciseCode} theme="vs-dark" />
                  <button onClick={runExercise} disabled={runningCode} className="mt-4 px-6 py-3 bg-gradient-to-r from-indigo-600 to-blue-600 text-white rounded-xl font-semibold hover:from-indigo-700 hover:to-blue-700 transition-all disabled:opacity-50 flex items-center gap-2">
                    <Play className="w-4 h-4" />{runningCode ? 'Running...' : 'Run Code'}
                  </button>
                  {exerciseResult && (
